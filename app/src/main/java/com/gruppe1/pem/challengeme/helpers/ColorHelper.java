...
public class ColorHelper {
   ...
   public static com.gruppe1.pem.challengeme.Color getClosestColor(int color,
         List<com.gruppe1.pem.challengeme.Color> allColors) {
      double minDist = 1000;
      com.gruppe1.pem.challengeme.Color closestColor = null;
      for (int i = 0; i < allColors.size(); i++) {
         double dist = colorDistance(color, Color.parseColor(allColors.get(i)
               .getHexColor()));
         if (dist < minDist) {
            minDist = dist;
   ...
   public static com.gruppe1.pem.challengeme.Color getClosestColor(String color,
         List<com.gruppe1.pem.challengeme.Color> allColors) {
   ...
   public static Drawable filterIconColor(Context context, String icon, int colorHex) {
      int iconId = context.getResources()
            .getIdentifier(icon, "drawable", "com.gruppe1.pem.challengeme");
      Drawable iconDrawable = context.getDrawable(iconId);
      if (iconDrawable != null) {
         iconDrawable.mutate()
               .setColorFilter(colorHex, PorterDuff.Mode.MULTIPLY);
      }
      return iconDrawable;
   }
}
