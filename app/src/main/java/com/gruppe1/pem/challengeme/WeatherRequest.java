...
public class WeatherRequest {
   ...
   public WeatherRequest(Context context, LinearLayout noWeatherAvailableView,
         LinearLayout weatherAvailableView, ImageView weatherImageView, TextView weatherDescView,
         TextView weatherTempView, ImageView weatherImg1, ImageView weatherImg2,
         ImageView weatherImg3, TextView day1, TextView day2, TextView day3, TextView temp1,
         TextView temp2, TextView temp3, TextView navDrawerWeatherDate) {
            ...
            try {
               ...
               if (!Objects.equals(lastLocationRequest, "")) {
                  ...
                  long differenceInMinutes =
                        TimeUnit.MINUTES.convert(diffInMillies, TimeUnit.MILLISECONDS);
   ...
   private void doNewWeatherRequest() {
      ...
      new HttpAsyncTask().execute(
            "http://api.openweathermap.org/data/2.5/weather?q=" + location[0] + "," + location[1] +
                  "&lang=" + language + "&APPID=" + Constants.OWM_API_KEY);
      new HttpAsyncTask().execute(
            "http://api.openweathermap.org/data/2.5/forecast?q=" + location[0] + "," + location[1] +
                  "&lang=" + language + "&APPID=" + Constants.OWM_API_KEY);
   }
   ...
   private String[] getGPS() {
      ...
      locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 3600000, 1000,
            locationListener); // all 1h, more than 1000 meters
      ...
      try {
         if (!Objects.equals(weather_date, "") && weather_date != null) {
            ...
            if (differenceInDays > 0) {
               ...
               navDrawerWeatherDate.setText(
                     context.getString(R.string.weatherold) + " " + differenceInDays + " " +
                           context.getString(R.string.weatherold2) +
                           ((differenceInDays > 1) ? context.getString(R.string.weatherold22) :
                                 "") + " " + context.getString(R.string.weatherold3) + ".");
            } else {
      ...
      protected void onPostExecute(String result) {
         ...
         try {
            ...
            if (json.has("weather")) {
               ...
               weather_today_desc = weather.getJSONObject(0)
                     .getString("description");
               weather_today_temp = Math.round(json.getJSONObject("main")
                     .getDouble("temp") - 273.15) + "°C";
               weather_today_image = getImage(weather.getJSONObject(0)
                     .getString("id"));
               ...
               if (json.has("list")) {
                  ...
                  for (int i = 0; i < forecast.length(); i++) {
                     int weather_dt = Integer.parseInt(forecast.getJSONObject(i)
                           .getString("dt"));
                     ...
                     if (!segs[0].equals(currentDateandTime)) {
                        if (segs[1].equals("13")) {
                           ...
                           JSONArray weather = forecast.getJSONObject(i)
                                 .getJSONArray("weather"); // get articles array
                           String temp = Math.round(forecast.getJSONObject(i)
                                 .getJSONObject("main")
                                 .getDouble("temp") - 273.15) + "°C";
                           String code = weather.getJSONObject(0)
                                 .getString("id");
